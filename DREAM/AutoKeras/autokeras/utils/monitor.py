...
def check_weights(weight_list):
    # delete not float type weight
    new_weight_list=[]
    for weight in weight_list:
        if 'float32' == weight.dtype or 'float64'== weight.dtype or 'float16'== weight.dtype:
...
def loss_issue(feature_dic,history,total_epoch,satisfied_acc,checkgap,\
    unstable_threshold=0.05,judgment_point=0.2,unstable_rate=0.25,epsilon=10e-3,sc_threshold=0.01):
    ...
    if train_loss!=[]:
        ...
        judg=min(judgment_point*total_epoch,3)
        if current_epoch>=judg:
...
def gradient_issue(feature_dic,gradient_list,threshold_low=1e-3,threshold_low_1=1e-4,threshold_high=70,threshold_die_1=0.7):
    ...
    if (gra_rate<threshold_low and norm_kernel[0]<threshold_low_1):
        ...
        feature_dic['vanish_gradient']=0
    if(gra_rate>threshold_high):
        feature_dic['explode_gradient']+=1
    else:
        feature_dic['explode_gradient']=0
...
class IssueMonitor:
    ...
    # unstable_threshold=0.05,judgment_point=0.3,unstable_rate=0.25,epsilon=10e-3,sc_threshold=0.01):
        ...
        # if self.issue_list !=[]:
        #     print(1)
        # self.issue_list = ['relu']
        # from random import choice
        # self.issue_list = [choice(['relu','explode','vanish','not_converge','unstable'])]
