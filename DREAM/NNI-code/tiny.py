...
import nni
from utils_data import cifar100_load_data
from utils import *
...
def tiny_load_data(num_classes=200):
    tf.compat.v1.enable_eager_execution()
    tiny_imagenet_builder = TinyImagenetDataset(data_dir='./1_dataset')
    dataset = tiny_imagenet_builder.as_dataset(as_supervised=True)

    x_train = np.array([example[0] for example in tfds.as_numpy(dataset['train'])])
    y_train = np.array([example[1] for example in tfds.as_numpy(dataset['train'])])
    y_train = np.eye(num_classes)[y_train]


    x_test = np.array([example[0] for example in tfds.as_numpy(dataset['validation'])])
    y_test = np.array([example[1] for example in tfds.as_numpy(dataset['validation'])])
...
def main(params_dict,
    ...
    (x_train, y_train), (x_test, y_test)=tiny_load_data()#cifar100_load_data()
    ...
    data,_=extract_dataset_np([x_train[:batch_size,...],y_train[:batch_size]],tmp_dir=tmp_dir,method='tiny')
    history=model.fit(
        x_train,
        y_train,
...
def convert_search_space(hyperparams,search_space_path='./search_space-tiny.json'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir','-sd',default='./demo_result', help='searching result path')
    ...
    parser.add_argument('--hypermodel_path','-hp',default='./hypermodel-tiny.pkl', type=str, help='hypermodel_path')
