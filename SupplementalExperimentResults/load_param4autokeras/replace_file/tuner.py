...
class AutoTuner(kerastuner.engine.tuner.Tuner):
    ...
    def _build_and_fit_model(self, trial, fit_args, fit_kwargs):
        ...
        with open(fit_kwargs['root_path'], 'rb') as f:#input,bug type,params
            save_dict = pickle.load(f)
        save_dict_path=fit_kwargs['root_path']
        del fit_kwargs['root_path']

        root_path=save_dict['root_path']
        log_path=save_dict['log_path']
        trial_num_path=save_dict['num_path']
        tuner=save_dict['tuner']

        with open(log_path, 'rb') as f:#input,bug type,params
            log_dict = pickle.load(f)
        
        # if log_dict['cur_trial']==-1:
        #     hyperpara_dir=os.path.join(root_path,'0-param')
        #     os.makedirs(hyperpara_dir)
        #     hyperparam_path=os.path.join(hyperpara_dir,'param.pkl')

        #     with open(hyperparam_path, 'wb') as f:
        #         pickle.dump(trial.hyperparameters, f)
        time_usage=time.time()-log_dict['start_time']
        if log_dict['cur_trial']==-1:
            #     # only use to evaluate tuners.
            if not os.path.exists(log_dict['origin_param_path']):
                with open(log_dict['origin_param_path'], 'wb') as f:
                    pickle.dump(trial.hyperparameters, f)
            else:  
                with open(log_dict['origin_param_path'], 'rb') as f:#input,bug type,params
                    ak_params = pickle.load(f)
                # modify
                # ak_params.values['learning_rate']=ak_params.values['learning_rate']*10
                trial.hyperparameters=ak_params
                self.oracle.hyperparameters=ak_params


        elif log_dict['cur_trial']>=25 and time_usage>=86400:
            ...
            new_dir='{}_{}'.format(root_path,tuner)
        ...
        # min_acc=min(max(train_history['accuracy']),max(train_history['val_accuracy']))
