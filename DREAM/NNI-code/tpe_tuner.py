...
class TpeTuner(Tuner):
    ...
    **Implementation**
    ...
    def update_search_space(self, space):
        # print(space)#2024xy
        # 2024xy
        if 'hyper_path' in space.keys():
            self.hyper_path=space['hyper_path']
            del space['hyper_path']
        self.origin_space=space
        # print(space)

        self.space = format_search_space(space)
        self.dedup = Deduplicator(self.space)
        # print('tpe update_search_space')#2024xy
    ...
    def generate_parameters(self, parameter_id, **kwargs):
        ...
        # print('======history======')#2024xy
        # print(history)
        if len(history)==0:# 2024xy # read from history
            hyper_path=self.hyper_path#'/home/zxy/main/DL_autokeras/test_codes/FORM/2024_nni_test/param.pkl'
            search_path=self.origin_space#'/home/zxy/main/DL_autokeras/test_codes/FORM/2024_nni_test/search_space-c100.json'
            print(f'Read From {hyper_path}')

            with open(hyper_path, 'rb') as f:
                hyperparameters = pickle.load(f)
            unformatted_params=convert_search_space(hyperparameters,search_path)
            # print(unformatted_params)
            params=format_parameters(unformatted_params, self.space)
            # print(params)
            ...
            params = suggest(self.args, self.rng, self.space, history)
        # print('tpe generate_parameters: suggest')#2024xy
        params = self.dedup(params)

        self._params[parameter_id] = params
        # print('tpe generate_parameters: parameter_id')#2024xy
...
## 2024xy
def convert_search_space(hyperparams,search_space_path='./search_space-c100.json'): #2024xy
    ...
    if isinstance(search_space_path,str):
...
if __name__=="__main__":
    hyper_path='/home/zxy/main/DL_autokeras/test_codes/FORM/2024_nni_test/param.pkl'
    search_path='/home/zxy/main/DL_autokeras/test_codes/FORM/2024_nni_test/search_space-c100.json'
    print(f'Read From {hyper_path}')
    import pickle
    import json
    with open(hyper_path, 'rb') as f:
